...
// TODO: Possible addition, ID support for people using later versions?
public enum BattleMaterial {
    ...
    BattleMaterial(int data, String... materials) {
    ...
    public boolean isDamageable(BattleMaterial type){
        if (type == null)
        	return false;
    ...
    public boolean isSameMaterial(ItemStack comp){
        ...
        BattleMaterial mat = fromMaterial(comp.getType());
    ...
    private static HashMap<String, BattleMaterial> cachedSearch = new HashMap<String, BattleMaterial>();

    public static BattleMaterial fromMaterial(Material mat){
        try {
            return BattleMaterial.valueOf(mat.toString());
        } catch (IllegalArgumentException ex) {
            for (BattleMaterial battleMat : BattleMaterial.values()){
                for (String test : battleMat.materials) {
    ...
    public static BattleMaterial fromString(String key){
        ...
        try {
            ...
            if (split.length == 1)
                mat = requestBattleMaterial(matKey,  0);
            else
                mat = requestBattleMaterial(split[0], Integer.parseInt(split[1]));

            return mat;
        }
    }

    public static BattleMaterial requestBattleMaterial(String name, int data){
        if (cachedSearch.containsKey(name.toUpperCase() + "," + data))
            return cachedSearch.get(name.toUpperCase() + "," + data);

        for (BattleMaterial mat : BattleMaterial.values()) {
